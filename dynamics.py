...
def utils_format_labels(labels, clean=False, min_area=9):
    """
    ...
    labels = labels.astype(
        "int32"
    )  # no one is going to have more than 2^32 -1 cells in one frame, right?
            ...
            if len(regions) > 1:
                ...
                for rg in regions[1:]:
                    if rg.area <= min_area:
                        ...
                        print(
                            "secondary disjoint part bigger than min_area, relabeling. Area:",
                            rg.area,
                            "Label value:",
                            np.unique(labels[rg.coords[:, 0], rg.coords[:, 1]]),
                        )
    ...
    fastremap.renumber(
        labels, in_place=True
    )  # convenient to have unit increments from 1 to N cells
    labels = fastremap.refit(labels)  # put into smaller data type if possible
    return labels


@njit(
    "(float64[:], int32[:], int32[:], int32[:], int32[:], int32, int32, boolean,float64)",
    nogil=True,
)
    ...
    if not skel:  # do original centroid projection algrorithm
        ...
        centers = np.array(
            scipy.ndimage.center_of_mass(
                masks_padded,
                labels=masks_padded,
                index=np.arange(1, masks_padded.max() + 1),
            )
        ).astype(int)
        # (check mask center inside mask)
        valid = masks_padded[centers[:, 0], centers[:, 1]] == np.arange(
            1, masks_padded.max() + 1
        )
...
def masks_to_flows(masks, use_gpu=False, device=None, skel=False):
    ...
    """
    masks = utils_format_labels(masks)
    dists = edt.edt(masks)
    ...
    if masks.ndim == 3:
        ...
        for x in range(Lx):
            mu0 = masks_to_flows_device(
                masks[:, :, x], dists, device=device, skel=skel
            )[0]
            mu[[0, 1], :, :, x] += mu0
        ...
        if skel:  # padding helps avoid edge artifacts from cut-off cells
            ...
            mu, T = masks_to_flows_device(
                masks_pad, dists_pad, device=device, skel=skel
            )
...
def labels_to_flows(
    labels, files=None, use_gpu=False, device=None, skel=False, redo_flows=False
):
    ...
    if (
        ...
        labels, dist, heat, veci = map(
            list,
            zip(
                *[
                    masks_to_flows(
                        labels[n][0], use_gpu=use_gpu, device=device, skel=skel
                    )
                    for n in range(nimg)
                ]
            ),
...
def steps2D_interp(
    p, dP, niter, use_gpu=False, device=None, skel=False, calc_trace=False
):
    shape = dP.shape[1:]
    if use_gpu and TORCH_ENABLED:
        if device is None:
            device = torch_GPU
        shape = (
            np.array(shape)[[1, 0]].astype("double") - 1
        )  # Y and X dimensions (dP is 2.Ly.Lx), flipped X-1, Y-1
        pt = (
            torch.from_numpy(p[[1, 0]].T).double().to(device).unsqueeze(0).unsqueeze(0)
        )  # p is n_points by 2, so pt is [1 1 2 n_points]
        im = (
            torch.from_numpy(dP[[1, 0]]).double().to(device).unsqueeze(0)
...
@njit(
    "(float32[:,:,:], float32[:,:,:], int32[:,:], int32, boolean, boolean)", nogil=True
)
...
def follow_flows(
    ...
    if len(shape) > 2:
        ...
        if skel:
            inds = (
                np.array(np.nonzero(np.logical_or(mask, np.abs(dP[0]) > 1e-3)))
                .astype(np.int32)
                .T
            )
        else:
...
def metrics_flow_error(maski, dP_net, use_gpu=False, device=None, skel=True):
    ...
    # ensure unique masks
    maski = np.reshape(
        np.unique(maski.astype(np.float32), return_inverse=True)[1], maski.shape
    )

    # flows predicted from estimated masks
    idx = -1  # flows are the last thing returned now
    dP_masks = masks_to_flows(maski, use_gpu=use_gpu, device=device, skel=skel)[idx]
...
def remove_bad_flow_masks(
    masks, flows, threshold=0.4, use_gpu=False, device=None, skel=False
):
    ...
    """
    merrors, _ = metrics_flow_error(masks, flows, use_gpu, device, skel)
...
def get_masks(
    p, iscell=None, rpad=20, flows=None, threshold=0.4, use_gpu=False, device=None
):
    ...
    if M0.max() > 0 and threshold is not None and threshold > 0 and flows is not None:
        M0 = remove_bad_flow_masks(
            M0, flows, threshold=threshold, use_gpu=use_gpu, device=device
        )
        _, M0 = np.unique(M0, return_inverse=True)
